...
addSubmission(submissions, "Rachel", 85, "2020-10-21");
...
const findSubmissionsByName = (array, name) => {
  return array.find((arrayStudentName) => {
    return arrayStudentName.name === name;
  });
};
console.log(findSubmissionsByName(submissions, "Jack"));
...
const filterPassing = (array) => {
  let passing = array.filter((students) => {
    return students.passed === true;
  });
  return passing;
};
// filterPassing(submissions); Call it
...
const filter90AndAbove = (array) => {
  let scoreGreater = array.filter((students) => {
